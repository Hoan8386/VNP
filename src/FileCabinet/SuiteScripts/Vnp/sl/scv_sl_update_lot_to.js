                            ...
                            for (let lotAvailable of listLotAvailable) {
                                ...
                                if (remainQuantiy <= 0) {
                                    ...
                                    if (remainQuantiy > quantityavailable) {
                                        libFunc.setSublistValueData(recSubIVD, slIVD, ['issueinventorynumber', 'quantity', 'binnumber'], lineIvd, [lotAvailable.inventorynumber, quantityavailable, lotAvailable.binnumber]);
                                        remainQuantiy = remainQuantiy - quantityavailable;
                                        lotAvailable.quantityavailable = 0;
                                    } else {
                                        libFunc.setSublistValueData(recSubIVD, slIVD, ['issueinventorynumber', 'quantity', 'binnumber'], lineIvd, [lotAvailable.inventorynumber, remainQuantiy, lotAvailable.binnumber]);
            ...
            if (objParams.locations) {
                ...
                let sqlItemAvailable = `
					select ib.item, ib.inventorynumber,
						(ib.quantityavailable - ib.committedqtyperseriallotnumberlocation + ib.quantitypicked) quantityavailable, ib.binnumber, ib.location, in.expirationdate, in.inventorynumber inventorynumber_name
					...
					where (ib.quantityavailable - ib.committedqtyperseriallotnumberlocation + ib.quantitypicked) > 0 and l.makeinventoryavailable = 'T' and ib.location in(${inLocations}) and ib.item in (${inItems}) and is.inventoryavailable = 'T'
