        ...
        const getListItemsAvailable = (objParams) => {
            ...
            if (objParams.locations) {
                ...
                let sqlItemAvailable = `
                    select
                        a.*,
                        in.inventorynumber as inventorynumber_name,
                        in.expirationdate                        
                    from (
                        select
                            ib.item,
                            ...
                            max(ib.committedqtyperseriallotnumberlocation) as committedqtyperseriallotnumberlocation,
                    
                            sum(ib.quantityonhand)
                                - max(ib.committedqtyperseriallotnumberlocation) as quantityavailable
                    
                        from inventorybalance ib
                        join location l
                            on ib.location = l.id
                        left join inventorystatus ist
                            on ib.inventorystatus = ist.id
                        where
                            l.makeinventoryavailable = 'T'
                            and ist.inventoryavailable = 'T'
                            and ib.location in (${inLocations})
                            and ib.item in (${inItems})
                        group by
                            ib.item,
                            ib.inventorynumber,
                            ib.location
                    ) a
                    left join inventorynumber in
                        on a.inventorynumber = in.id
                    where
